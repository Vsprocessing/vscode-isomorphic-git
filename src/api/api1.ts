...
export class ApiRepository implements Repository {
	...
	constructor(repository: BaseRepository) {
		...
		this.rootUri = fileUri(this.#repository.root);
	...
	buffer(ref: string, filePath: string): Promise<Buffer> {
		return this.#repository.buffer(ref, filePath) as Promise<Buffer>;
	}
	...
	add(paths: string[]) {
		return this.#repository.add(paths.map(p => fileUri(p)));
	}

	revert(paths: string[]) {
		return this.#repository.revert(paths.map(p => fileUri(p)));
	}

	clean(paths: string[]) {
		return this.#repository.clean(paths.map(p => fileUri(p)));
	}

	restore(paths: string[], options?: { staged?: boolean; ref?: string }) {
		return this.#repository.restore(paths.map(p => fileUri(p)), options);
	}
...
export class ApiImpl implements API {
	...
	async getRepositoryWorkspace(uri: Uri): Promise<Uri[] | null> {
		...
		return workspaces ? workspaces.map(r => fileUri(r.workspacePath)) : null;
	}
	...
	async clone(uri: Uri, options?: CloneOptions): Promise<Uri | null> {
		...
		return result ? fileUri(result) : null;
	}

	async openRepository(root: Uri): Promise<Repository | null> {
		if (!isWorkspaceFileUri(root)) {
			return null;
		}
