...
import { getExtensionContext } from './web/runtime';
...
import { EMPTY_TREE, Repository as BaseRepository, BlameInformation, Commit, CommitShortStat, GitError, IDotGit, LogFileOptions, LsTreeElement, PullOptions, RefQuery, Stash, Submodule, Worktree } from './git';
...
import { fileUri, gitFs, isWorkspaceFileUri } from './web/runtime';

const timeout = (millis: number) => new Promise(c => setTimeout(c, millis));

function getIconUri(iconName: string, theme: string): Uri {
	return Uri.joinPath(getExtensionContext().extensionUri, 'resources', 'icons', theme, `${iconName}.svg`);
}
...
export class Resource implements SourceControlResourceState {
	...
	// Built lazily: icon URIs need the extension context, which is not available at module load on the web.
	private static _icons: ReturnType<typeof createStatusIcons> | undefined;
	private static get Icons(): ReturnType<typeof createStatusIcons> {
		return Resource._icons ??= createStatusIcons();
	}
...
class ProgressManager {
	...
	constructor(private repository: Repository) {
		const onDidChange = filterEvent(workspace.onDidChangeConfiguration, e => e.affectsConfiguration('git', fileUri(this.repository.root)));
	...
	private updateEnablement(): void {
		const config = workspace.getConfiguration('git', fileUri(this.repository.root));
...
class DotGitWatcher implements IFileWatcher {
	...
	constructor(
		...
		const filteredRootWatcher = filterEvent(rootWatcher.event, uri => isWorkspaceFileUri(uri) && !/\/\.git(\/index\.lock|\/worktrees\/[^/]+\/index\.lock)?$|\/\.watchman-cookie-/.test(uri.path));
...
class ResourceCommandResolver {
	...
	resolveCompareWithWorkspaceCommand(resource: Resource): Command {
		...
		// Worktrees are not supported in the browser, so there is no parent checkout to compare with.
		const leftUri: Uri | undefined = candidateFsPath ? undefined : undefined;
...
export class Repository implements Disposable {
	...
	constructor(
		...
		const repositoryWatcher = workspace.createFileSystemWatcher(new RelativePattern(fileUri(repository.root), '**'));
		...
		const root = fileUri(repository.root);
	...
	diffBetweenWithStats(ref1: string, ref2: string, path?: string): Promise<DiffChange[]> {
		...
		const scopedConfig = workspace.getConfiguration('git', fileUri(this.root));
			...
			await this.run(
				...
				async () => {
					...
					if (opts.requireUserConfig === undefined || opts.requireUserConfig === null) {
						const config = workspace.getConfiguration('git', fileUri(this.root));
	...
	private async commitOperationCleanup(message: string | undefined, indexResources: string[], workingGroupResources: string[]) {
		...
		const resources = indexResources.length !== 0
			? indexResources.map(r => fileUri(r))
			: workingGroupResources.map(r => fileUri(r));
		commands.executeCommand('_chat.editSessions.accept', resources);
	...
	async _clean(resources: string[]): Promise<void> {
		...
		if (discardUntrackedChangesToTrash) {
			try {
				...
				await workspace.fs.delete(fileUri(resources[0]), { useTrash: true });

				const limiter = new Limiter<void>(5);
				await Promise.all(resources.slice(1).map(fsPath => limiter.queue(
					async () => await workspace.fs.delete(fileUri(fsPath), { useTrash: true }))));
			} catch {
	...
	closeDiffEditors(indexResources: string[] | undefined, workingTreeResources: string[] | undefined, ignoreSetting = false): void {
		...
		function checkTabShouldClose(input: TabInputTextDiff | TabInputNotebookDiff) {
			...
			if (isWorkspaceFileUri(input.modified) && input.original.scheme === 'git' && (workingTreeResources === undefined || workingTreeResources.some(r => pathEquals(r, input.modified.fsPath)))) {
				// Working Tree
				return true;
			}
			return false;
	...
	private async _getWorktreeIncludePaths(): Promise<Set<string>> {
		...
		const filePattern = worktreeIncludeFiles
			.map(pattern => new RelativePattern(fileUri(this.root), pattern));
		...
		try {
			...
			const results = await Promise.allSettled(files.map(sourceFile => {
				return limiter.queue(async () => {
					...
					await workspace.fs.copy(fileUri(sourceFile), fileUri(targetFile), { overwrite: true });
				});
			}));
	...
	async checkout(treeish: string, opts?: { detached?: boolean; pullBeforeCheckout?: boolean }): Promise<void> {
		const refLabel = opts?.detached ? getCommitShortHash(fileUri(this.root), treeish) : treeish;
	...
	async show(ref: string, filePath: string): Promise<string> {
		return await this.run(Operation.Show, async () => {
			try {
				...
				return await workspace.decode(content, { uri: fileUri(filePath) });
			} catch (err) {
	...
	async ignore(files: Uri[]): Promise<void> {
		return await this.run(Operation.Ignore, async () => {
			...
			const document = await gitFs.exists(ignoreFile)
				? await workspace.openTextDocument(fileUri(ignoreFile))
				: await workspace.openTextDocument(fileUri(ignoreFile).with({ scheme: 'untitled' }));
	...
	checkIgnore(filePaths: string[]): Promise<Set<string>> {
		return this.run(Operation.CheckIgnore, () => this.repository.checkIgnore(filePaths.filter(filePath => isDescendant(this.root, filePath))));
	}
	...
	private async findKnownHugeFolderPathsToIgnore(): Promise<string[]> {
		...
		for (const folderName of Repository.KnownHugeFolderNames) {
			...
			if (await gitFs.exists(folderPath)) {
	...
	private async getStatus(cancellationToken?: CancellationToken): Promise<GitResourceGroups> {
		...
		const scopedConfig = workspace.getConfiguration('git', fileUri(this.repository.root));
		...
		if (didHitLimit && !shouldIgnore && !this.didWarnAboutLimit) {
			...
			if (knownHugeFolderPaths.length > 0) {
				...
				window.showWarningMessage(`${gitWarn} ${addKnown}`, yes, no, neverAgain).then(result => {
					if (result === yes) {
						this.ignore([fileUri(folderPath)]);
					} else {
		...
		status.forEach(raw => {
			const uri = fileUri(path.join(this.repository.root, raw.path));
			const renameUri = raw.rename
				? fileUri(path.join(this.repository.root, raw.rename))
				: undefined;
	...
	private isCherryPickInProgress(): Promise<boolean> {
		...
		return gitFs.exists(cherryPickHeadPath);
	}
