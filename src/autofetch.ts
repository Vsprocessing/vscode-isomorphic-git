...
import { workspace, Disposable, EventEmitter, Memento, window, MessageItem, ConfigurationTarget, ConfigurationChangeEvent, l10n, env } from 'vscode';
...
export class AutoFetcher {
	...
	private async onFirstGoodRemoteOperation(): Promise<void> {
		...
		if (result === yes) {
			const gitConfig = workspace.getConfiguration('git', fileUri(this.repository.root));
	...
	private async run(): Promise<void> {
		while (this.enabled) {
			...
			const period = workspace.getConfiguration('git', fileUri(this.repository.root)).get<number>('autofetchPeriod', 180) * 1000;
