...
import { basename, normalize, sep, relative } from 'path';
import { fileUri, gitFs } from './web/runtime';
...
export const isLinuxSnap = false;
...
export async function mkdirp(path: string, _mode?: number): Promise<boolean> {
	await gitFs.promises.mkdir(path).catch(async () => {
		const stat = await gitFs.promises.stat(path);
		if (!stat.isDirectory()) {
...
export async function grep(filename: string, pattern: RegExp): Promise<boolean> {
	const content = new TextDecoder().decode(await workspace.fs.readFile(fileUri(filename)));
